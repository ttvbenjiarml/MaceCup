...
while((Get-Date) -lt $deadline -and -not $p.HasExited){
  ...
  if(Test-Path $logPath){
    ...
    if($log -match 'SELFTEST ALL PASSED'){$passed=$true;break}
    if($log -match 'SELFTEST FAIL|SEVERE|ERROR|Exception|Could not load|Failed to start'){$failed=$true;break}
  }
}
...
$result=if($failed){'FAILED'}elseif($passed){'PASSED'}else{'TIMEOUT'}
